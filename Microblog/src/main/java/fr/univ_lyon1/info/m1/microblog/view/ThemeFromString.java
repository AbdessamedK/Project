 ...
 */
public class ThemeFromString {
    private static final String PACKAGE = "fr.univ_lyon1.info.m1.microblog.view.";
     ...
     */
    public ThemeStrategy createStrategy(final String strategyName) {
        try {
            
            String className = PACKAGE + strategyName;
            Class<?> strategy = Class.forName(className);
            return (ThemeStrategy) strategy.getDeclaredMethod("getInstance").invoke(null);
